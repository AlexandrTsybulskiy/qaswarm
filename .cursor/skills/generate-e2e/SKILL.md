...
4. **Sync Playwright root first** (see `.cursor/rules/code-repos.mdc` → Sync Playwright before e2e write): in `<playwright_root>`, `git fetch origin` then `git pull --ff-only` on the tracking branch. If dirty tree / non-ff pull / unexpected local changes — stop and ask; never hard-reset without explicit human choice. Note tip commit to the builder (or that sync failed).
5. Launch `e2e-builder` with testdoc path, product id, playwright root, optional `run_command`, and that sync was done (or failed). If the Task harness has no `e2e-builder` type, use `generalPurpose` instructed to follow `.cursor/agents/e2e-builder.md` verbatim. Builder writes `_incoming/` only.
6. After MANIFEST lists `e2e.md`:
...
7. Report paths, missing/written counts, and pass/fail/blocked/skipped if run exists. Say e2e is ready only if `e2e/<slug>.md` has `status: ready`, `e2e-runs/<slug>.md` exists, and `py -3 tools/memory_schema.py memory/<id>` would pass.
...
8. Never write to Upservice or Testmo. Do not modify `runs/`. Do not start generate-testdocs, analyze-requirement, or verify-testdocs from this skill.
